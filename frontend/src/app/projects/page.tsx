...
export default function GlobalProjectsOverviewPage() {
    ...
    const channelLimitReached = (channels?.length ?? 0) >= (me?.channel_limit ?? 1);

    return (
        <div className="max-w-5xl mx-auto p-8">
            <header className="flex flex-col md:flex-row md:justify-between items-start md:items-end gap-6 mb-10 pt-8">
                <div>
                    <h2 className="text-4xl font-manrope font-bold text-primary tracking-tight leading-none">Your Projects</h2>
                    <p className="text-muted-foreground mt-4 font-semibold text-xs uppercase tracking-widest">Manage your podcast feeds and sync your content across platforms.</p>
                </div>
                ...
                <section className="mb-12">
                    <div className="relative overflow-hidden rounded-2xl bg-surface-container shadow-[0px_24px_48px_rgba(25,28,30,0.06)] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8 group">
                        ...
                        <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-6 w-full relative z-10">
                            ...
                            <div className="pt-2">
                                <h3 className="text-2xl font-manrope font-bold text-primary">Connect your YouTube Account</h3>
                                <p className="text-muted-foreground max-w-xl text-base mt-2">Before creating a project, link your YouTube account so we can access your channels and sync your video uploads to podcast feeds.</p>
                            </div>
                        </div>
                        <Link
                            href="/onboarding"
            ...
            ) : !channelsLoading && !channelsError && (channels?.length ?? 0) === 0 ? (
                <section className="mb-12">
                    <div className="relative overflow-hidden rounded-2xl bg-surface-container shadow-[0px_24px_48px_rgba(25,28,30,0.06)] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8 group">
                        ...
                        <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-6 w-full relative z-10">
                            ...
                            <div className="pt-2">
                                <h3 className="text-2xl font-manrope font-bold text-primary">Connect your YouTube Channel</h3>
                                <p className="text-muted-foreground max-w-xl text-base mt-2">Automatically sync your video uploads to your podcast feeds and reach a wider audience across Spotify, Apple, and Google.</p>
                            </div>
                        </div>
                        <Link
                            href="/projects/new"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        {statsLoading ? (
                            ...
                            <>
                                <div className="flex justify-between items-start mb-4">
                                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Total Downloads</p>
                                ...
                                <p className="text-xs font-medium text-muted-foreground mt-3 uppercase tracking-wider">All time</p>
                            </>
                        )}
                    </div>
                    ...
                    <div className="bg-surface-container-lowest p-6 md:p-8 rounded-2xl shadow-[0px_24px_48px_rgba(25,28,30,0.06)] hover:-translate-y-1 transition-transform">
                        {statsLoading ? (
                            ...
                            <>
                                <div className="flex justify-between items-start mb-4">
                                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Total Episodes</p>
                                    <span className="flex items-center text-[10px] font-bold text-accent bg-accent/10 px-2 py-1 rounded-md">
                                        Live
                                    </span>
                                </div>
                                <p className="text-5xl font-manrope font-bold text-primary tracking-tight">{totalEpisodes}</p>
                                <p className="text-xs font-medium text-muted-foreground mt-3 uppercase tracking-wider">Across all projects</p>
                            </>
                        )}
                    </div>
                    ...
                    <div className="bg-surface-container-lowest p-6 md:p-8 rounded-2xl shadow-[0px_24px_48px_rgba(25,28,30,0.06)] hover:-translate-y-1 transition-transform">
                        <div className="flex justify-between items-start mb-4 w-full">
                            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Connected Projects</p>
                            <span className={`flex items-center text-[10px] font-bold px-2 py-1 rounded-md ${channelsLoading ? 'text-muted-foreground bg-muted' : 'text-accent bg-accent/10'}`}>
                                {channelsLoading ? '—' : 'Live'}
                            </span>
                        </div>
                        <p className="text-5xl font-manrope font-bold text-primary tracking-tight">{totalChannels}</p>
                    </div>
                </div>
            </section>
            ...
            <section className="pb-20">
                <div className="flex items-center justify-between mb-8">
                    <h3 className="text-3xl font-manrope font-bold text-primary">Active Projects</h3>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse min-w-[800px]">
                            <thead className="bg-surface-container-low border-b border-border">
                                <tr>
                                    <th className="px-6 md:px-8 py-5 text-xs font-semibold text-muted-foreground uppercase tracking-widest">Project</th>
                                    <th className="px-6 md:px-8 py-5 text-xs font-semibold text-muted-foreground uppercase tracking-widest">Status</th>
                                    <th className="px-6 md:px-8 py-5 text-xs font-semibold text-muted-foreground uppercase tracking-widest">Episodes</th>
                                    <th className="px-6 md:px-8 py-5 text-xs font-semibold text-muted-foreground uppercase tracking-widest text-right">Dashboard</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-border">
                                ...
                                {channelsError && (
                                    <tr>
                                        <td colSpan={4} className="px-8 py-16 text-center text-sm font-semibold text-destructive">
                                ...
                                {!channelsLoading && !channelsError && channels?.length === 0 && (
                                    <tr>
                                        <td colSpan={4} className="px-8 py-20 text-center">
                                            <div className={`size-16 rounded-full flex items-center justify-center mx-auto mb-6 ${isYouTubeConnected ? 'bg-muted' : 'bg-[#FF0000]/10'}`}>
                                                {isYouTubeConnected
                                                    ? <Rss className="size-8 text-muted-foreground" />
                                                    : <Youtube className="size-8 text-[#FF0000]" />}
                                            </div>
                                            <p className="text-lg font-semibold text-primary mb-2">
                                                {isYouTubeConnected ? 'No projects connected yet' : 'YouTube not connected'}
                                            </p>
                                            ...
                                            >
                                                {isYouTubeConnected ? 'Connect a Channel' : 'Connect YouTube'}
                                            </Link>
                                        </td>
                                    </tr>
                                )}
                                ...
                                {channels?.map((channel) => (
                                    <tr key={channel.id} className="hover:bg-surface-container group transition-colors">
                                        <td className="px-6 md:px-8 py-6">
                                            <div className="flex items-center gap-4 md:gap-5">
                                                ...
                                                <Link href={`/projects/${channel.id}`} className="font-bold text-primary group-hover:text-accent transition-colors text-lg md:text-xl line-clamp-2">
                                        ...
                                        <td className="px-6 md:px-8 py-6">
                                            <span className="flex items-center gap-2 text-sm font-medium whitespace-nowrap">
                                                <span className={`size-2.5 rounded-full ${channel.monitoring_active ? 'bg-[#5516be] shadow-[0_0_8px_rgba(85,22,190,0.5)]' : 'bg-muted-foreground'}`} />
